...
- Root cause: the Artwork Review area contained forms nested inside WooCommerce's existing order-edit form. Nested forms are invalid HTML, so browsers could submit the outer WooCommerce form instead of the artwork action.
...
- Existing review history and WooCommerce Order Notes remain intact.
