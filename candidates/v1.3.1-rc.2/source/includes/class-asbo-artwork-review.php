...
final class ASBO_Artwork_Review {
    ...
    private static function rights_confirmed( WC_Order $order ): bool {
        return 'yes' === sanitize_key( (string) $order->get_meta( self::META_RIGHTS_CONFIRMED, true ) );
    }

    private static function record_rights_confirmation( WC_Order $order, string $source ): void {
        $statement = class_exists( 'ASBO_Plugin' ) ? ASBO_Plugin::artwork_rights_statement() : __( 'I confirm that I own this artwork or have permission or a license to reproduce and use it for this order.', 'all-star-bulk-order' );
        $version = class_exists( 'ASBO_Plugin' ) ? ASBO_Plugin::artwork_rights_version() : '2026-09-11-v1';

        $order->update_meta_data( self::META_RIGHTS_CONFIRMED, 'yes' );
        $order->update_meta_data( self::META_RIGHTS_VERSION, sanitize_text_field( $version ) );
        $order->update_meta_data( self::META_RIGHTS_ACCEPTED_AT, gmdate( 'c' ) );
        $order->update_meta_data( self::META_RIGHTS_USER_ID, get_current_user_id() );
        $order->update_meta_data( self::META_RIGHTS_STATEMENT, sanitize_textarea_field( $statement ) );
        $order->update_meta_data( self::META_RIGHTS_SOURCE, sanitize_key( $source ) );
    }
    ...
    private static function render_upload_form( WC_Order $order, string $return_url, bool $revision ): void {
        ...
        <form class="asbo-artwork-form" method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
            ...
            <?php if ( self::rights_confirmed( $order ) ) : ?>
                <p class="asbo-artwork-rights-confirmed"><strong>✓ <?php esc_html_e( 'Artwork-use authorization confirmed for this order.', 'all-star-bulk-order' ); ?></strong></p>
            <?php else : ?>
                <label class="asbo-artwork-rights-check">
                    <input type="checkbox" name="asbo_artwork_rights" value="1" required>
                    <span><strong><?php esc_html_e( 'Artwork-use authorization', 'all-star-bulk-order' ); ?></strong><br><?php echo esc_html( class_exists( 'ASBO_Plugin' ) ? ASBO_Plugin::artwork_rights_statement() : __( 'I confirm that I own this artwork or have permission or a license to reproduce it for this order.', 'all-star-bulk-order' ) ); ?><small><?php esc_html_e( 'Finding an image online does not by itself grant permission to reproduce it.', 'all-star-bulk-order' ); ?></small></span>
                </label>
            <?php endif; ?>
    ...
    public static function handle_upload(): void {
        ...
        $record_rights_on_success = ! self::rights_confirmed( $order );
        if ( $record_rights_on_success ) {
            $rights_value = isset( $_POST['asbo_artwork_rights'] ) ? sanitize_text_field( wp_unslash( $_POST['asbo_artwork_rights'] ) ) : '';
            if ( '1' !== $rights_value ) {
                self::redirect_error( $return_url, __( 'Confirm that you own the artwork or have permission to reproduce it before uploading.', 'all-star-bulk-order' ) );
            }
        }
        ...
        if ( $record_rights_on_success ) {
            self::record_rights_confirmation( $order, 'upload' );
        }
        ...
        if ( $record_rights_on_success ) {
            $order->add_order_note( 'Customer confirmed artwork-use authorization during artwork upload (' . sanitize_text_field( $order->get_meta( self::META_RIGHTS_VERSION, true ) ) . ').' );
        }
    ...
    private static function ensure_protected_artwork_directory(): bool {
    ...
    public static function render_review_metabox( $post_or_order ): void {
        ...
        $rights_confirmed = self::rights_confirmed( $order );
        $rights_version = sanitize_text_field( (string) $order->get_meta( self::META_RIGHTS_VERSION, true ) );
        $rights_accepted_at = sanitize_text_field( (string) $order->get_meta( self::META_RIGHTS_ACCEPTED_AT, true ) );
            ...
            <div class="asbo-review-admin__grid">
                ...
                <div class="asbo-review-admin__details">
                    ...
                    <dl>
                        ...
                        <div><dt><?php esc_html_e( 'Artwork rights', 'all-star-bulk-order' ); ?></dt><dd><?php if ( $rights_confirmed ) : ?><strong style="color:#24734a">✓ <?php esc_html_e( 'Confirmed', 'all-star-bulk-order' ); ?></strong><?php if ( $rights_accepted_at ) : ?> · <?php echo esc_html( self::format_datetime( $rights_accepted_at ) ); ?><?php endif; ?><?php if ( $rights_version ) : ?> <small>(<?php echo esc_html( $rights_version ); ?>)</small><?php endif; ?><?php else : ?><strong style="color:#a04435"><?php esc_html_e( 'Not recorded (legacy order)', 'all-star-bulk-order' ); ?></strong><?php endif; ?></dd></div>
                    </dl>
